...
function heapSort( array ) {
    for ( let i = array.length - 1; i >= 0; i-- ) { //-call heapify and pass in the length of the array and the same i that represents the current node... we are heapifying sub trees that make up the
...
function heapify( array, n, i ) { //array is same as this.array; i is same as i; n will denote where heap ends and sorted part of array begins.
    ...
    //if leftChild or rightchild index lie outside of the heap.... set to neg infinity so can do safe comparison
    if ( leftIdx >= n ) leftVal = -Infinity;
    if ( rightIdx >= n ) rightVal = -Infinity;

    //* if current node already has maxHeap value relative to it's children nodes.
