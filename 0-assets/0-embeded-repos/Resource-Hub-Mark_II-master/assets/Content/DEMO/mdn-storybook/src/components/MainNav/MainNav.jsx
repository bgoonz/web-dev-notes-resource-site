...
const _MainNav = ({ document, locale }: Props) => {
          ...
          <li key={menuEntry.label} className="top-level-entry-container">
            <button
              ...
              onTouchStart={() => {
                ...
                // JavaScript.
                setShowSubMenu(
                  showSubMenu === menuEntry.label ? null : menuEntry.label
                );
              }}
            >
              {menuEntry.label}
              <span className="main-menu-arrow" aria-hidden="true">
                ▼
              </span>
            </button>
            <ul className={menuEntry.label === showSubMenu ? "show" : null}>
              {menuEntry.items.map(item => (
                <li key={item.url} data-item={menuEntry.label} role="menuitem">
                  {item.external ? (
                    <a
                      target="_blank"
                      rel="noopener noreferrer"
                      href={item.url.replace("{{PATH}}", path)}
                    >
                      {item.label} &#x1f310;
                    </a>
                  ) : (
...
const MainNav = memo<Props>(_MainNav);
