...
const DataTableCatalog = ( props ) => {
  return ( <
    ComponentCatalogPanel hero = {
      < DataTableHero / >
    }
    title = 'Data Table'
    description = 'Data tables display information in a way that'
    s easy to scan, so that users can look
...
const Checkbox = ( props ) => {
  const classes = classNames( 'mdc-checkbox', {
    'mdc-checkbox--selected': props.isChecked
  }, props.classes );
  const {
    ariaLabel,
    ariaLabelledby
...
const HeaderRowCheckboxCell = ( props ) => {
  ...
  return ( <
    HeaderCell classes = {
      extraCssClasses.HEADER_CELL_CHECKBOX
    } >
    <
    Checkbox classes = {
      cssClasses.HEADER_ROW_CHECKBOX
    }
    aria - label = {
      ariaLabel
    }
...
const RowCheckboxCell = ( props ) => {
  ...
  return ( <
    Cell classes = {
      extraCssClasses.CELL_CHECKBOX
    } >
    <
    Checkbox classes = {
      cssClasses.ROW_CHECKBOX
    }
    isChecked = {
      props.isChecked
    }
...
const DataTable = ( props ) => {
    ...
    return ( <
        ...
        tr className = {
          ...
          props.data.headers.map( ( header, index ) => < HeaderCell key = {
              index
            } > {
              header
            } < /HeaderCell>)} <
              ...
              props.data.rows.map( ( row, index ) => {
                  ...
                  return ( <
                    ...
                    key = {
                      ...
                      props.rowSelection ? < RowCheckboxCell ariaLabelledby = {
                        rowId
                      }
                      isChecked = {
                        isSelected
          ...
          const DataTableHero = () => < DataTable data = {
            sampleData
          }
          />;
          const DataTableDemos = () => ( <
            ...
            h4 className = {
              'demo-data-table-header'
            } > Data Table with Row Selection < /h4> <
            DataTable data = {
              sampleRowSelectionData
            }
            rowSelection / >
            <
            /div>
