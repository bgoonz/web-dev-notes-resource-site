...
(function ($) {
  ...
  var $window = $(window),
    ...
    _templateMessage = function ($form, title, errorMessages, conf) {
      ...
      container = conf.errorMessageTemplate.container.replace(
        /\{errorMessageClass\}/g,
  ...
  $.fn.validateInputOnBlur = function (
    ...
    if (result.isValid) {
      if (result.shouldChangeDisplay) {
        $elem.addClass("valid");
        _getInputParentContainer($elem).addClass(
          conf.inputParentClassOnSuccess
        );
      }
    } else if (!result.isValid) {
      _applyErrorStyle($elem, conf);
      _setInlineErrorMessage(
        $elem,
        result.errorMsg,
        conf,
  ...
  $.fn.isValid = function (language, conf, displayError) {
    ...
    if (displayError) {
      ...
      _removeErrorStyle(
        $form.find("." + conf.errorElementClass + ",.valid"),
        conf
      );
    }
  ...
  $.split = function (val, callback) {
    if (typeof callback != "function") {
      ...
      var values = [];
      $.each(
        val.split(callback ? callback : /[,|\-\s]\s*/g),
        function (i, str) {
          str = $.trim(str);
    ...
    $(conf.form).each(function (i, form) {
      ...
      $form
        ...
        .bind("reset.validation", function () {
          // remove messages
          $(this)
            .find("." + conf.errorMessageClass + ".alert")
            .remove();
          _removeErrorStyle(
            $(this).find("." + conf.errorElementClass + ",.valid"),
            conf
          );
        })
    ...
    if (conf.modules != "") {
      $.formUtils.loadModules(conf.modules, false, function () {
        ...
        $window.trigger("validatorsLoaded", [
          typeof conf.form == "string" ? $(conf.form) : conf.form,
          conf,
        ]);
      });
    }
  ...
  $.formUtils = {
    ...
    validateInput: function ($elem, language, conf, $form, eventContext) {
      ...
      if (validateIfCheckedElementName != null) {
        ...
        validateIfCheckedElement = $form.find(
          'input[name="' + validateIfCheckedElementName + '"]'
        );
  ...
  $.formUtils.addValidator({
    ...
    validatorFunction: function (val, $el, conf, lang) {
      ...
      switch (
        ...
        case "out":
          this.errorMessage =
            lang.lengthBadStart + lengthAllowed + lang.lengthBadEnd;
          checkResult = false;
          break;
        // too short
        case "min":
