...
function PMSColorMatching( C, B ) {
  B || ( B = 32 );
  var D = new Array,
    A = new Array;
  m = RGB2PMS( C ), "" != m && ( D[ 0 ] = m ), r = parseInt( C.substr( 0, 2 ), 16 ), g = parseInt( C.substr( 2, 2 ), 16 ), b = parseInt( C.substr( 4, 2 ), 16 );
  for ( var E = 0; E < aRGB.length; E++ ) {
    rgb1 = aRGB[ E ];
    r1 = parseInt( rgb1.substr( 0, 2 ), 16 );
    g1 = parseInt( rgb1.substr( 2, 2 ), 16 ), b1 = parseInt( rgb1.substr( 4, 2 ), 16 );
    A[ E ] = Math.sqrt( Math.pow( r - r1, 2 ) + Math.pow( g - g1, 2 ) + Math.pow( b - b1, 2 ) );
  }
  for ( E = 0; E < A.length; E++ ) A[ E ] <= B && -1 == D.indexOf( aPMS[ E ] ) && D.push( aPMS[ E ] );
  return D
}

function generatePantoneList( hex ) {
  let localHex, dist, A, F, E;
  localHex = hex;
  dist = $( '#distance' ).val();
  A = PMSColorMatching( localHex, dist );
  E = '';
  F = 0;
  if ( 0 < A.length ) {
    E += '';
    ...
    for ( var e = 0; e < A.length; e++ ) {
      E += '<div style="width: 120px; float: left; margin-left: 10px;"><div style="width: 100%;height: 30px; text-align:Center;">';
      if ( F < A.length ) {
        rgbcode = PMS2RGB( A[ F ] );
        E += A[ F ] + '</div> <div title=\'' + rgbcode + '\' style=\'background-color:#' + rgbcode + ';height:120px;width:100%;\'>&nbsp;';
        F += 1;
      }
      E += '</div></div>';
    }
...
function RgbToCmyk( rr, gg, bb ) {
  var c, m, y, k, rc, rm, ry, rk, r, g, b, rr, gg, bb;
  r = rr;
  g = gg;
  b = bb;
  if ( r == "" ) r = 0;
  ...
  ry = roundnum( y, 2 );
  rk = roundnum( k, 2 );
  if ( populateCMYKFromRGB ) {
    $( '#c' ).val( rc );
    $( '#m' ).val( rm );
    $( '#y' ).val( ry );
    $( '#k' ).val( rk );
  }
  populateCMYKFromRGB = true;
}

function drawColorPalette() {
  colorPicker = $( "#colorPalette" ).colpick( {
    flat: true,
    layout: "hex",
    submit: 0,
    color: {
    ...
    onChange: function ( e, h, a ) {
      $( '#colorEffect' ).css( "background-color", "rgb(" + a.r + ", " + a.g + ", " + a.b + ")" )
      RgbToCmyk( a.r, a.g, a.b );
      generatePantoneList( h );
    }
  } );
}

function cmyk2rgb() {
  var c, m, y, k, r, g, b, rgbVal, response;
  c = $( '#c' ).val();
  m = $( '#m' ).val();
  y = $( '#y' ).val();
  k = $( '#k' ).val();
...
$( document ).ready( function () {
  ...
  $( '#c,#m,#y,#k,#distance' ).change( function () {
    populateCMYKFromRGB = false;
    let rgbColor;
    rgbColor = cmyk2rgb();
    colorPicker.colpickSetColor( rgbColor );
  } );
} )
$( '#c, #m, #y, #k' ).keyup( function () {
  let c = $( '#c' ).val();
  let m = $( '#m' ).val();
  let y = $( '#y' ).val();
  let k = $( '#k' ).val();
