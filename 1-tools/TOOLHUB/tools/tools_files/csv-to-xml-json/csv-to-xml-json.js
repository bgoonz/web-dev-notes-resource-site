...
function convertToJson() {
  let input, inputVal, linesLength;
  input = $( '#input-text' );
  inputVal = input.val();
  if ( inputVal.trim() == '' ) return false;

  for ( var e = CSVToArray( inputVal ), n = [], g = 1; g < e.length; g++ ) {
    n[ g - 1 ] = {};
    for ( var a = 0; a < e[ 0 ].length && a < e[ g ].length; a++ ) {
      var l = e[ 0 ][ a ];
      n[ g - 1 ][ l ] = e[ g ][ a ]
    }
  }
  res = JSON.stringify( n ).replace( /},/g, "},\r\n" );

  resulteditor.matchBrackets = true;
  resulteditor.autoCloseBrackets = true;
  resulteditor.mode = "application/ld+json"
  resulteditor.lineWrapping = true;
  resulteditor.setValue( res );
