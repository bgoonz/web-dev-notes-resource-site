...
function convertToHex() {
  ...
  $( '#colorEffect' ).css( "background-color", "rgb(" + r + ", " + g + ", " + b + ")" );
  $( '#h' ).val( hex );
  var c = RgbToCmyk( r, g, b );
  $( '#copyrgb' ).attr( "data-clipboard-text", "rgb(" + r + ", " + g + ", " + b + ")" );
}
...
function cmyk2rgb() {
  ...
  $( '#colorEffect' ).css( "background-color", "rgb(" + r + ", " + g + ", " + b + ")" );
  rgbVal = "rgb(" + r + "," + g + "," + b + ")";
  $( '#copyrgb' ).attr( "data-clipboard-text", rgbVal );
}
...
function rgb2hsv( e, h, a ) {
  hsv_red = Number( e ) / 255, hsv_green = Number( h ) / 255, hsv_blue = Number( a ) / 255;
  var m, r, b = Math.max( hsv_red, hsv_green, hsv_blue ),
    c = Math.min( hsv_red, hsv_green, hsv_blue ),
    n = b,
    g = b - c;
  if ( r = 0 == b ? 0 : g / b, b == c ) m = 0;
  else {
    switch ( b ) {
      case hsv_red:
        m = ( hsv_green - hsv_blue ) / g + ( hsv_green < hsv_blue ? 6 : 0 );
        break;
      case hsv_green:
        m = ( hsv_blue - hsv_red ) / g + 2;
        break;
      case hsv_blue:
        m = ( hsv_red - hsv_green ) / g + 4
    }
    m /= 6
  }
  return m.toFixed( 4 ) + "," + r.toFixed( 4 ) + "," + n.toFixed( 4 )
}
...
function drawColorTable() {
  var b, table, cmykVal;
  b = [];
  $.each( colors, function ( e, h ) {
    var a = "<div style='height:20px;margin:0 auto;width:100%;border:1px solid;background-color:" + h.hex + "'></div>",
      m = h.rgb.split( "," ),
      r = rgb2hsv( m[ 0 ], m[ 1 ], m[ 2 ] );
    r = r.split( "," ), b[ e ] = [ a, h.name, h.hex, h.rgb, r[ 0 ] + "," + r[ 1 ] + "," + r[ 2 ], h.cmyk ]
  } );
  ...
  $( ".sourceCodeOptionDiv :input" ).keyup( function ( e ) {
    13 == ( e.keyCode || e.which ) && $( "#convertbtn" ).click()
  } );
  $( "#colorPalette" ).colpick( {
    ...
    onChange: function ( e, h, a ) {
      ...
      $( '#g' ).val( a.g );
      $( '#b' ).val( a.b );
      var c = RgbToCmyk( a.r, a.g, a.b );
      c = c.split( "," ), $( "#c" ).val( c[ 0 ] ), $( "#m" ).val( c[ 1 ] ), $( "#y" ).val( c[ 2 ] ), $( "#k" ).val( c[ 3 ] );
      $( '#colorEffect' ).css( "background-color", "rgb(" + a.r + ", " + a.g + ", " + a.b + ")" )
      var rgb_val = "rgb(" + a.r + ", " + a.g + ", " + a.b + ")";
      $( '#copyrgb' ).attr( 'data-clipboard-text', rgb_val );
    }
  } )
  $( '#colorTable>thead' ).addClass( 'thead-dark' );
}
...
var clipboards = new ClipboardJS( '#copyrgb' );
